...
% spath = 'D:\Data\Kelton\analyses\KW058\KW058_08262025_rec_D5_LLat2'; % 10m rest
% spath = 'D:\Data\Kelton\analyses\KW060\KW060_08282025_rec_D5_LLat2'; % 10m rest
...
% spath = 'D:\Data\Kelton\analyses\KW066\KW066_09182025_rec_D4_LLat2'; % 10m rest
...
if saveFlag
    ...
    fsave(bhvrFig,[root.name '_bhvrPrePost'])
    fsave(ppVelFig,[root.name '_prepostVel'])
    fsave(preVelMapF,[root.name,'_preVelMap'])
    fsave(pstVelMapF,[root.name,'_pstVelMap'])
    fsave(ppLckFig,[root.name '_prepostLck'])
...
colormap('hot')
frstHalf.spVelMap = plot_lap_velCCorr(sessFrst,vspbnsz,0.002,0);
lastHalf.spVelMap = plot_lap_velCCorr(sessLast,vspbnsz,0.002,0);

% Correlate each trial to the associated average map
% preVelAvg = mean(sessFrst.spVelMap);
% pstVelAvg = mean(sessLast.spVelMap);
...
% figure; hold on; plot(smooth(preVCorXLap)); plot(smooth(pstVCorXLap));

% % Mohamed method compare 100cm pre RZ
% prepreRZVelAvg = [mean(preVelMap(1:end-1,0.96/vspbnsz:1.85/vspbnsz)) mean(preVelMap(2:end,0.01/vspbnsz:r1pos/vspbnsz))];
% prepreRZVel = [preVelMap(1:end-1,0.96/vspbnsz:1.85/vspbnsz) preVelMap(2:end,0.01/vspbnsz:r1pos/vspbnsz)];
% pstpreRZVel = pstVelMap(:,0.01/vspbnsz:1/vspbnsz);
% 
% preVCorXLap = corr(prepreRZVel',prepreRZVelAvg','rows','complete');
% pstVCorXLap = corr(pstpreRZVel',prepreRZVelAvg','rows','complete');
% figure; hold on; plot(preVCorXLap); plot(pstVCorXLap);
% figure; hold on; plot(smooth(preVCorXLap)); plot(smooth(pstVCorXLap));
...
plot(frstHalf.vCorXLap,'Color',vColors2(1,:)); 
plot(lastHalf.vCorXLap,'Color',vColors2(2,:)); % plot(smooth(preVCorXLap),'r','LineWidth',2); plot(smooth(pstVCorXLap),'b','LineWidth',2);
plot(2.5:5:length(frstHalf.vCorAvg)*5,frstHalf.vCorAvg,'Color',vColors2(1,:),'LineWidth',2); 
plot(2.5:5:length(lastHalf.vCorAvg)*5,lastHalf.vCorAvg,'Color',vColors2(2,:),'LineWidth',2)
legend({'Pre-shift','Post-shift'},'Location','se'); 
ylabel('Velocity Correlation'); xlabel('Lap'); 
set(gca,'FontSize',16,'FontName','Arial')

if saveFlag
    fsave(vXCorFig,[root.name '_velXCorr'])
    fsave(vXCorLapsFig,[root.name '_velXCorrXLaps'])
...
[~,frMap2] = get_frXpos(rootLast,cc,sessLast,dbnsz);
[ciup2, cidn2] = get_CI(frMap2);

frFig = figure; hold on
xcoords = 100*binpos;
set(gcf,'units','normalized','position',[0.4 0.35 0.2 0.20])
plot_CIs(xcoords,ciup1,cidn1,'k')
plot(xcoords,mean(frMap1),'k')
plot_CIs(xcoords,ciup2,cidn2,'r')
plot(xcoords,mean(frMap2),'r')
ymax = round(max([max(mean(frMap1),[],'all') max(mean(frMap2),[],'all')]))+2;
ylim([0 ymax]); yticks([0 ymax]);
xlim([0 max(xcoords)])
...
plot(1:size(frstHalf.frMap,1),smoothdata(uPVPrePre),'b')
plot(1:size(lastHalf.frMap,1),smoothdata(uPVPstPre),'r')
...
mdlVPreXPVCprepre = get_linfit(frstHalf.vCorXLap,frstHalf.pvXlap(2:end,1));
mdlVPreXPVCprepst = get_linfit(frstHalf.vCorXLap,frstHalf.pvXlap(2:end,2));
mdlVPstXPVCpstpre = get_linfit(lastHalf.vCorXLap,lastHalf.pvXlap(:,1));
mdlVPstXPVCpstpst = get_linfit(lastHalf.vCorXLap,lastHalf.pvXlap(:,2));

pvcXvcorF = figure; hold on
plot(frstHalf.vCorXLap,frstHalf.pvXlap(2:end,1),'ko')
plot(frstHalf.vCorXLap,frstHalf.pvXlap(2:end,2),'o','Color',vColors2(1,:))
plot(lastHalf.vCorXLap,lastHalf.pvXlap(:,1),'ro')
plot(lastHalf.vCorXLap,lastHalf.pvXlap(:,2),'o','Color',[1 .6 .6])
plot(frstHalf.vCorXLap,mdlVPreXPVCprepre.ypred,'k','LineWidth',2)
plot(frstHalf.vCorXLap,mdlVPreXPVCprepst.ypred,'Color',vColors2(1,:),'LineWidth',2)
plot(lastHalf.vCorXLap,mdlVPstXPVCpstpre.ypred,'r','LineWidth',2)
plot(lastHalf.vCorXLap,mdlVPstXPVCpstpst.ypred,'Color',[1 .6 .6],'LineWidth',2)
xlim([-1 1]); xlabel('Velocity Correlation');
ylim([-1 1]); ylabel('PV Correlation');
legend({'preV->PrePrePVC','preV->PrePstPVC','pstV->PstPrePVC','pstV->PstPstPVC'},'location','se')
set(gca,'FontSize',12,'FontName','Arial')

if saveFlag
    saveas(pvcXvcorF,[sbase '_pvCorrXvCorr.png'])
end

%% PCA attempt

% nTrials = size(frstHalf.frMap,1);
% nBins = size(frstHalf.frMap,2);
% nUnits = size(frstHalf.frMap,3);
% 
% figure; hold on;
% 
% for i = 1:nTrials
%     clusterDat = normalize(squeeze(frstHalf.frMap(i,:,:)),'range');
%     [coeff,score,latent,tsquare,explained] = pca(clusterDat);
%     plot(score(:,1),score(:,2),'Color',[.5 .5 .5])
% end
% 
% clusterDat = normalize(frstHalf.posfr','range');
% [coeff,score,latent,tsquare,explained] = pca(clusterDat);
% 
% plot(score(:,1),score(:,2),'k','LineWidth',2)
% plot(score(1,1),score(1,2),'bo')
% plot(score(nBins,1),score(nBins,2),'ro')
% 
% figure;
% plot(explained)
...
idx = find(ismember(subDatT.fpath, spath));
borderDists = [subDatT.sh1dist(idx) subDatT.sh2dist(idx) subDatT.sh3dist(idx) subDatT.sh4dist(idx)];
[datXanatSIFig,frstHalf.d2cs,frstHalf.d2bs] = plot_datXlyr(root,frstHalf.trueSI,bothSIUnits,borderDists);
datXanatSIFig.Children(1).Label.String = 'Spatial Info. (bits/spike)';

[datXanatFRFig] = plot_datXlyr(root,root.info.fr(root.goodind),useUnits,borderDists);
datXanatFRFig.Children(1).Label.String = 'Firing Rate (Hz)';

[thAngs,~,thPs] = get_thAng(frstHalf.thetastats);
[datXanatFRFig] = plot_datXlyr(root,thAngs+180,thPs < 0.05,borderDists);
datXanatFRFig.Children(1).Label.String = 'Theta Phase \circ';
...
for i = 1:length(root.good)
    cc = root.good(i);
    [frstHalf.RRSI(i),~,~,~,frstHalf.rrFRMap(:,:,i),~,famQMap(i,:)] = get_SI_cue(rootFrst,cc,sessFrst,qfrst);
    % frstHalf.rrFRMap(:,:,i) = tmpspkmap ./ tmpoccmap;
    [lastHalf.RRSI(i),~,~,~,lastHalf.rrFRMap(:,:,i),~,novQMap(i,:)] = get_SI_cue(rootLast,cc,sessLast,qlast);
...
[frstWflF,~,sortInd] = plot_unitWaterfall(frstHalf.famQMap(bothSIUnits,:),binedges);
lastWflF = plot_unitWaterfall(lastHalf.novQMap(bothSIUnits,:),binedges,sortInd);
lastWflN = plot_unitWaterfall(lastHalf.novQMap(bothSIUnits,:),binedges);
...
posNormF = (squeeze(mean(frstHalf.rrFRMap(:,:,bothSIUnits),1)) ./ max(squeeze(mean(frstHalf.rrFRMap(:,:,bothSIUnits),1)),[],1))';
posNormR = (squeeze(mean(lastHalf.rrFRMap(:,:,bothSIUnits),1)) ./ max(squeeze(mean(lastHalf.rrFRMap(:,:,bothSIUnits),1)),[],1))';
pvPrePst = corr(posNormR,posNormF);   % units x sp. bins
pvPrePstCompF = plot_pvcorr(pvPrePst,[-92.5,0,92.5]); ylabel('Position (cm) Random RZ')

% % PVC F/R odd vs F/R even for visualization
frstHalf.pvOddRR = (squeeze(mean(frstHalf.rrFRMap(1:2:end,:,bothSIUnits),1,"omitnan")) ./ max(squeeze(mean(frstHalf.rrFRMap(1:2:end,:,bothSIUnits),1,"omitnan")),[],1))';
frstHalf.pvEvnRR = (squeeze(mean(frstHalf.rrFRMap(2:2:end,:,bothSIUnits),1,"omitnan")) ./ max(squeeze(mean(frstHalf.rrFRMap(1:2:end,:,bothSIUnits),1,"omitnan")),[],1))';
lastHalf.pvOddRR = (squeeze(mean(lastHalf.rrFRMap(1:2:end,:,bothSIUnits),1,"omitnan")) ./ max(squeeze(mean(lastHalf.rrFRMap(1:2:end,:,bothSIUnits),1,"omitnan")),[],1))';
lastHalf.pvEvnRR = (squeeze(mean(lastHalf.rrFRMap(2:2:end,:,bothSIUnits),1,"omitnan")) ./ max(squeeze(mean(lastHalf.rrFRMap(1:2:end,:,bothSIUnits),1,"omitnan")),[],1))';
pvPreOddEvn = corr(frstHalf.pvOddRR,frstHalf.pvEvnRR);
pvPstOddEvn = corr(lastHalf.pvOddRR,lastHalf.pvEvnRR);
pvPreOddEvnFig = plot_pvcorr(pvPreOddEvn,[-92.5,0,92.5]);
xlabel("Position (cm) even laps"); ylabel("Position (cm) odd laps")
pvPstOddEvnFig = plot_pvcorr(pvPstOddEvn,[-92.5,0,92.5]);
xlabel("Position (cm) even laps"); ylabel("Position (cm) odd laps")

% PVC x time
idMat = logical(eye(length(frstHalf.binedges)-1));
uPVPrePre = get_pvXtime(frstHalf.famQMap,frstHalf.rrFRMap,bothSIUnits);
uPVPrePst = get_pvXtime(lastHalf.novQMap,frstHalf.rrFRMap,bothSIUnits,idMat);
uPVPstPre = get_pvXtime(frstHalf.famQMap,lastHalf.rrFRMap,bothSIUnits,idMat);
uPVPstPst = get_pvXtime(lastHalf.novQMap,lastHalf.rrFRMap,bothSIUnits);
frstHalf.pvXlapRR = [uPVPrePre, uPVPrePst];
lastHalf.pvXlapRR = [uPVPstPre, uPVPstPst];

if saveFlag
    save([root.name '_RwdShift_Data2'],'frstHalf','lastHalf')
    fsave(pvPrePstCompF,[sbase '_RwdRand_PVC_FvsR'],1,1)
    fsave(pvPreOddEvnFig,[sbase '_RwdRand_PVC_OddEvn_F'],1,1)
    fsave(pvPstOddEvnFig,[sbase '_RwdRand_PVC_OddEvn_R'],1,1)
end
