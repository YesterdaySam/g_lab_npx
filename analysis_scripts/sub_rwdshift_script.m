...
hmFig = plot_trialHeatmap(root,cc,sess,0.05,1,1,1,5); hold on;
plot([1 length(binpos)],[rwdShift rwdShift],'w--','LineWidth',2);
set(gcf,'units','normalized','position',[0.4 0.35 0.2 0.4])
set(gca,'FontSize',16,'FontName','Arial')

[~,frMap1] = get_frXpos(rootFrst,cc,sessFrst);
[ciup1, cidn1] = get_CI(frMap1);
[~,frMap2] = get_frXpos(rootLast,cc,sessLast);
...
if saveFlag
    ...
    saveas(hmFig,[sbase 'heatmap'], 'svg')
    saveas(frFig,[sbase 'frXpos'],'svg')
end
...
thAngFig = plotBar2(thDat(thBothID,3),thDat(thBothID,6));
...
if saveFlag
    saveas(pkPosPatchFig,[sbase 'lc_PeakComp_patch'],'png')
end
...
ylim([-0.25 1]); ylabel('Mean of PV Corr.')
set(gca,'FontSize',16,'FontName','Arial')

if saveFlag
    saveas(pvFig,[sbase 'lc_pv_corr_allSICells'],'png')
    saveas(pvCompFig,[sbase 'lc_pv_comp_allSICells'],'png')
end
...
pvXlapPre = NaN(maxTr(1),nMice);
pvXlapPst = NaN(maxTr(2),nMice);

for i = 1:nMice
    pvXlapPre(1:nTrialsPre(i),i) = pvStr(i).preBlockPV(:,1);
    pvXlapPst(1:nTrialsPst(i),i) = pvStr(i).pstBlockPV(:,1);
end
...
for i = 1:nMice
    plot(1:maxTr(1),pvXlapPre(:,i),'Color',cmapcool(i,:))
end
plot(1:maxTr(1),nanmean(pvXlapPre'),'k','LineWidth',2)
for i = 1:nMice
    plot(101:100+maxTr(2),pvXlapPst(:,i),'Color',cmapcool(i,:))
end
ylim([-0.25 1]); ylabel('PV Corr. on-diagonal'); xlabel('Lap #')
plot(101:100+maxTr(2),nanmean(pvXlapPst'),'k','LineWidth',2)
xticks(00:20:200)
...
if saveFlag
    saveas(pvXlapFig,[sbase 'lc_pv_corr_laps'],'png')
    saveas(pvXlapFig,[sbase 'lc_pv_corr_laps'],'svg')
end
...
swrBothPreSortPreHisto = plot_unitPkHisto(tmpMap,binedges); xlim([-wlen wlen])
...
swrBothPstSortPstHisto = plot_unitPkHisto(tmpMap,binedges); xlim([-wlen wlen])
xlabel('Time to SWR peak (ms)')
...
datXanatThAngFig.Children(1).Label.String = 'Theta phase \circ'; clim([0 360])
...
xlabel('Theta phase \circ'); xlim([0 360])
...
datXanatSIFig.Children(1).Label.String = 'Spatial Info. (bits/spike)';
siProxDistFig = plot_proxVdist(lcDat(:,2),siBothID & proxCC,siBothID & distCC);
ylabel('Spatial Info. (bits/spike)')
...
rrCells = double(rrCells);
datXanatRFFig = plot_group_datXlyr(rrCells,siBothID,recID(:,4:5),winter(sum(siBothID)));
datXanatRFFig.Children(1).Ticks = [0 1]; colormap(redbluecmap(3));
datXanatRFFig.Children(1).TickLabels = {'Non-RR','RR'};
rfProxDistFig = plot_proxVdist(rrCells,siBothID & proxCC,siBothID & distCC);
ylabel('Spatial Info. (bits/spike)')
rfDrslVtrlFig = plot_drslVvtrl(rrCells,siBothID & drslCC,siBothID & vtrlCC);
xlabel('Spatial Info. (bits/spike)')
rfPDCorrFig = plot_anatCorr(rrCells(siBothID),recID(siBothID,5),1,winter(sum(siBothID)));
ylabel('Spatial Info. (bits/spike)')
rfDVCorrFig = plot_anatCorr(rrCells(siBothID),recID(siBothID,4),2,parula(sum(siBothID)));
xlabel('Spatial Info. (bits/spike)')

if saveFlag
    saveas(datXanatThAngFig,[sbase 'anat_thAng_PDDV_pre'],'png')
    saveas(thAngProxDistFig,[sbase 'anat_thAng_PDbar_pre'],'png')
    saveas(thAngDrslVtrlFig,[sbase 'anat_thAng_DVbar_pre'],'png')
    saveas(thAngPDCorrFig,[sbase 'anat_thAng_PDcor_pre'],'png')
    saveas(thAngDVCorrFig,[sbase 'anat_thAng_DVcor_pre'],'png')
    saveas(datXanatFRFig,[sbase 'anat_frRun_PDDV_pre'],'png')
    saveas(frProxDistFig,[sbase 'anat_frRun_PDbar_pre'],'png')
    saveas(frDrslVtrlFig,[sbase 'anat_frRun_DVbar_pre'],'png')
    saveas(frPVCorrFig,[sbase 'anat_frRun_PDcor_pre'],'png')
    saveas(frDVCorrFig,[sbase 'anat_frRun_DVcor_pre'],'png')
    saveas(datXanatSIFig,[sbase 'anat_si_PDDV_pre'],'png')
    saveas(siProxDistFig,[sbase 'anat_si_PDbar_pre'],'png')
    saveas(siDrslVtrlFig,[sbase 'anat_si_DVbar_pre'],'png')
    saveas(siPDCorrFig,[sbase 'anat_si_PDcor_pre'],'png')
    saveas(siDVCorrFig,[sbase 'anat_si_DVcor_pre'],'png')
    saveas(datXanatRFFig,[sbase 'anat_refFrame_PDDV'],'png')
end
