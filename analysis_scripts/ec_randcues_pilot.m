...
posFRMap = [];
for i = 1:length(siUnits)
    [binedges,posFRMap(i,:)] = plot_frXpos(root,root.good(siUnits(i)),sess,0.025,0,0);
end
...
[spWflFig,tmpMap] = plot_unitWaterfall(posFRMap,binedges);
title(replace(root.name,"_"," "))

[peakDistroFig] = plot_unitPkHisto(tmpMap,binedges*100); xlim([-inf inf])
...
%% Waterfall of rand cue responses
rndFRMap = [];
fixFRMap = [];
for i = 1:length(siUnits)
    [binedges,rndFRMap(i,:)] = plot_align2rand(root,root.good(siUnits(i)),sess,0,0.025,0);
    [binedges,fixFRMap(i,:)] = plot_align2rand(root,root.good(siUnits(i)),sess,1,0.025,0);
end

[fixWflF,tmpMap,fixSort] = plot_unitWaterfall(fixFRMap,binedges);
[fixWflFHisto] = plot_unitPkHisto(tmpMap,binedges*100); xlim([-inf inf])
[rndWflF,tmpMap] = plot_unitWaterfall(rndFRMap,binedges,fixSort);
[rndWflFHisto] = plot_unitPkHisto(tmpMap,binedges*100); xlim([-inf inf])

fsave(fixWflF,[sbase '_fixWaterfall'],1)
fsave(fixWflFHisto,[sbase '_fixPkHisto'],1)
fsave(rndWflF,[sbase '_rndWaterfall'],1)
fsave(rndWflFHisto,[sbase '_rndPkHisto'],1)
