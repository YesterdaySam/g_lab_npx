function [depthMap] = probeDepthMap(meta)
%%% channel depth map for LFP channel depth alignment

geomCell = strsplit(meta.snsGeomMap,')(');
geomCell = geomCell(2:end);
for i = 1:length(geomCell)
    siteParse = strsplit(geomCell{i},':');
    site.depth(i) = str2double(siteParse{3});
    % site.shank(i) = str2double(siteParse{1});
end
