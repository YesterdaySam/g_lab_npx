...
% Updated 4/3/26 LKW; Added functionality to merge multi file sessions
%--------------------------------------------------------------------------
...
% sess.ts         = [];
% % sess.vel        = [];
% sess.velshft    = []; % Subtract off velocity offset and convert analog voltage to cm/s, may leave negative values
% sess.pos        = [];
% sess.lck        = [];
% sess.opto       = [];
% sess.optoind    = [];
...
for i = 1:size(bhvrFile,1)
    ...
    sess.aidat      = [sess.aidat; dat.(datFields{2}).analogScans];
    sess.ind        = [sess.ind, sess.ind(end) + (1:size(sess.aidat,1))];
    % sess.ts         = [sess.ts, sess.ts(end) + (sess.ind/sess.samprate)];
    % sess.vel        = sess.aidat(:,1);
    sess.velshft    = (sess.aidat(:,1) - mode(sess.aidat(:,1)))*80; % Subtract off velocity offset and convert analog voltage to cm/s, may leave negative values
